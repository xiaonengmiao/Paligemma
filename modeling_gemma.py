...
class GemmaRMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        # Llama does x.to(float16) * w whilist Gemma is (x * w).to(float16)
...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Multiply by the values. [Batch_Size, Num_Heads_Q, Seq_Len_Q, Seq_Len_KV] x [Batch_Size, Num_Heads_Q, Seq_Len_KV, Head_Dim] -> [Batch_Size, Num_Heads_Q, Seq_Len_Q, Head_Dim]
        ...
        # Multiple by W_o. [Batch_Size, Seq_Len_Q, Hidden_Size]
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def forward(
        ...
        # [Batch_size, Num_Patches, Embed_Dim] -> [Batch_size, Num_Image_Tokens, Hidden_Size]
        images_features = self.multi_modal_projector(selected_image_feature)

        # Merge the embeddings of the text tokens and the image tokens
        inputs_embeds, attention_mask, position_ids = self._merge_input_ids_with_image_features(
            images_features, inputs_embeds, input_ids, attention_mask, kv_cache
        )
